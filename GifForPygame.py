...
class GIFImage(object):
    ...
    def render(self, screen, pos):
        if self.running:
            if time.time() - self.p_time > self.frames[self.cur][1]:
                if self.reversed:
                    self.cur -= 1
                    if self.cur < self.start_point:
                        self.cur = self.breakpoint
                else:
                    self.cur += 1
                    if self.cur > self.breakpoint:
                        self.cur = self.start_point
    ...
    def copy(self):
        ...
        new.start_point = self.start_point
        new.cur = self.cur
        new.p_time = self.p_time
